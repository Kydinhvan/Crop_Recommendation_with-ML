...
@app.route('/', methods=['GET', 'POST']) # we set both GET and POST, then in index() we define different handling for each type of method.
def index():
    if request.method == 'POST': # we load up all the input variables, calculate the predicted output then load that as well. then we create new row and commit it.
        ...
        db.session.commit()
        # go back to the original page (GET).
        return redirect('/')
    else:        
        # if the request is using GET method, then we show the page.
...
# clear DB for convenience.
def clear_database():
    db.session.query(AgriOutput).delete()
    db.session.commit()
    print("Database cleared: All records in AgriOutput have been deleted.")
